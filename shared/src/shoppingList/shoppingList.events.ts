...
const ShoppingListItemsAdded = HitpointsEvent.extend({
    type: z.literal('ShoppingListItemsAdded'),
    items: ShoppingListItem.array(),
    index: z.number().min(0).optional(),
});

const ShoppingListItemUpdated = HitpointsEvent.extend({
    type: z.literal('ShoppingListItemUpdated'),
    item: ShoppingListItem,
});

const ShoppingListItemsRemoved = HitpointsEvent.extend({
    type: z.literal('ShoppingListItemsRemoved'),
    itemIds: z.string().uuid().array(),
});

const ShoppingListItemsChecked = HitpointsEvent.extend({
    type: z.literal('ShoppingListItemsChecked'),
    itemIds: z.string().uuid().array(),
});

const ShoppingListItemsUnchecked = HitpointsEvent.extend({
    type: z.literal('ShoppingListItemsUnchecked'),
    itemIds: z.string().uuid().array(),
});

const ShoppingListItemMoved = HitpointsEvent.extend({
